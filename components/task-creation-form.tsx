...
import { createTask, type TaskCreationState } from "@/app/dashboard/agents/[id]/task-actions"
...
export default function TaskCreationForm({
  ...
  const [dependsOnAgentId, setDependsOnAgentId] = useState<string | null>(null)
  const [dependsOnTaskId, setDependsOnTaskId] = useState<string | null>(null)

  const initialState: TaskCreationState = {}
  const [state, formAction, isPending] = useActionState(
    (prevState: TaskCreationState | undefined, formData: FormData) => createTask(agentId, prevState, formData),
    initialState,
  )

  // Filter tasks based on selected dependsOnAgentId
  const filteredTasks = dependsOnAgentId
    ? allUserTasks.filter((task) => task.agent_id === dependsOnAgentId && task.id !== dependsOnTaskId)
    : []
  ...
  }, [dependsOnTaskId, dependsOnAgentId, allUserTasks, allUserAgents, isDependency, blockedReason])

  // Call onSuccess when task is created successfully
  if (state?.success && state.taskId && onSuccess) {
      ...
      <form action={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
            Add context, requirements, or specific instructions for this task.
          </p>
        </div>
          ...
          <>
            ...
            <div>
              <Label htmlFor="blockedReason" className="font-medium text-gray-700 dark:text-gray-300">
                Reason for Being Blocked / Human Approval
              </Label>
              <Textarea
                id="blockedReason"
                name="blockedReason"
                ...
                placeholder="Explain why this task needs human review or is blocked..."
                maxLength={200}
                className="mt-1 min-h-[80px]"
              />
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                This will appear in your Dependency Basket for review.
              </p>
            </div>
          </>
        )}
